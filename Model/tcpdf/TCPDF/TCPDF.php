...
class TCPDF
{
	...
	public function __construct($orientation='P', $unit='mm', $format='A4', $unicode=true, $encoding='UTF-8', $diskcache=false, $pdfa=false) {
		...
		$this->file_id = md5(TSTA::getRandomSeed('TCPDF'.$orientation.$unit.$serformat.$encoding));
		...
		TFON::utf8Bidi(array(''), '', false, $this->isunicode, $this->CurrentFont);
		// set default font
	...
	protected function setPageFormat($format, $orientation='P') {
		...
		if (is_string($format)) {
			...
			$pf = TSTA::getPageSizeFromFormat($format);
			$this->fwPt = $pf[0];
			$this->fhPt = $pf[1];
		} else {
			// the boundaries of the physical medium on which the page shall be displayed or printed
			if (isset($format['MediaBox'])) {
				$this->pagedim = TSTA::setPageBoxes($this->page, 'MediaBox', $format['MediaBox']['llx'], $format['MediaBox']['lly'], $format['MediaBox']['urx'], $format['MediaBox']['ury'], false, $this->k, $this->pagedim);
				...
				if (isset($format[0]) AND is_numeric($format[0]) AND isset($format[1]) AND is_numeric($format[1])) {
					...
					$pf = TSTA::getPageSizeFromFormat($format['format']);
				}
				$this->fwPt = $pf[0];
				$this->fhPt = $pf[1];
				$this->pagedim = TSTA::setPageBoxes($this->page, 'MediaBox', 0, 0, $this->fwPt, $this->fhPt, true, $this->k, $this->pagedim);
			}
			// the visible region of default user space
			if (isset($format['CropBox'])) {
				$this->pagedim = TSTA::setPageBoxes($this->page, 'CropBox', $format['CropBox']['llx'], $format['CropBox']['lly'], $format['CropBox']['urx'], $format['CropBox']['ury'], false, $this->k, $this->pagedim);
			}
			// the region to which the contents of the page shall be clipped when output in a production environment
			if (isset($format['BleedBox'])) {
				$this->pagedim = TSTA::setPageBoxes($this->page, 'BleedBox', $format['BleedBox']['llx'], $format['BleedBox']['lly'], $format['BleedBox']['urx'], $format['BleedBox']['ury'], false, $this->k, $this->pagedim);
			}
			// the intended dimensions of the finished page after trimming
			if (isset($format['TrimBox'])) {
				$this->pagedim = TSTA::setPageBoxes($this->page, 'TrimBox', $format['TrimBox']['llx'], $format['TrimBox']['lly'], $format['TrimBox']['urx'], $format['TrimBox']['ury'], false, $this->k, $this->pagedim);
			}
			// the page's meaningful content (including potential white space)
			if (isset($format['ArtBox'])) {
				$this->pagedim = TSTA::setPageBoxes($this->page, 'ArtBox', $format['ArtBox']['llx'], $format['ArtBox']['lly'], $format['ArtBox']['urx'], $format['ArtBox']['ury'], false, $this->k, $this->pagedim);
			}
	...
	public function setPageOrientation($orientation, $autopagebreak='', $bottommargin='') {
		...
		if (!isset($this->pagedim[$this->page]['CropBox'])) {
			// the visible region of default user space
			$this->pagedim = TSTA::setPageBoxes($this->page, 'CropBox', $this->pagedim[$this->page]['MediaBox']['llx'], $this->pagedim[$this->page]['MediaBox']['lly'], $this->pagedim[$this->page]['MediaBox']['urx'], $this->pagedim[$this->page]['MediaBox']['ury'], true, $this->k, $this->pagedim);
		}
		if (!isset($this->pagedim[$this->page]['BleedBox'])) {
			// the region to which the contents of the page shall be clipped when output in a production environment
			$this->pagedim = TSTA::setPageBoxes($this->page, 'BleedBox', $this->pagedim[$this->page]['CropBox']['llx'], $this->pagedim[$this->page]['CropBox']['lly'], $this->pagedim[$this->page]['CropBox']['urx'], $this->pagedim[$this->page]['CropBox']['ury'], true, $this->k, $this->pagedim);
		}
		if (!isset($this->pagedim[$this->page]['TrimBox'])) {
			// the intended dimensions of the finished page after trimming
			$this->pagedim = TSTA::setPageBoxes($this->page, 'TrimBox', $this->pagedim[$this->page]['CropBox']['llx'], $this->pagedim[$this->page]['CropBox']['lly'], $this->pagedim[$this->page]['CropBox']['urx'], $this->pagedim[$this->page]['CropBox']['ury'], true, $this->k, $this->pagedim);
		}
		if (!isset($this->pagedim[$this->page]['ArtBox'])) {
			// the page's meaningful content (including potential white space)
			$this->pagedim = TSTA::setPageBoxes($this->page, 'ArtBox', $this->pagedim[$this->page]['CropBox']['llx'], $this->pagedim[$this->page]['CropBox']['lly'], $this->pagedim[$this->page]['CropBox']['urx'], $this->pagedim[$this->page]['CropBox']['ury'], true, $this->k, $this->pagedim);
		}
		...
		if ((abs($this->pagedim[$this->page]['MediaBox']['urx'] - $this->hPt) < $this->feps) AND (abs($this->pagedim[$this->page]['MediaBox']['ury'] - $this->wPt) < $this->feps)){
			...
			$this->pagedim = TSTA::swapPageBoxCoordinates($this->page, $this->pagedim);
		}
		$this->w = ($this->wPt / $this->k);
		$this->h = ($this->hPt / $this->k);
		if (TSTA::empty_string($autopagebreak)) {
		...
		if (TSTA::empty_string($bottommargin)) {
	...
	public function SetDisplayMode($zoom, $layout='SinglePage', $mode='UseNone') {
		...
		$this->LayoutMode = TSTA::getPageLayoutMode($layout);
		$this->PageMode = TSTA::getPageMode($mode);
	}
	...
	public function Header() {
		if ($this->header_xobjid === false) {
			...
			if (($headerdata['logo']) AND ($headerdata['logo'] != K_BLANK_IMAGE)) {
				$imgtype = TIMA::getImageFileType(K_PATH_IMAGES.$headerdata['logo']);
	...
	protected function setTableHeader() {
		...
		if (!TSTA::empty_string($this->thead) AND (!$this->inthead)) {
			// set margins
	...
	public function setSpotColor($type, $name, $tint=100) {
		$spotcolor = TCOL::getSpotColor($name, $this->spot_colors);
		if ($spotcolor === false) {
	...
	public function GetStringWidth($s, $fontname='', $fontstyle='', $fontsize=0, $getarray=false) {
		return $this->GetArrStringWidth(TFON::utf8Bidi(TFON::UTF8StringToArray($s, $this->isunicode, $this->CurrentFont), $s, $this->tmprtl, $this->isunicode, $this->CurrentFont), $fontname, $fontstyle, $fontsize, $getarray);
	}
	...
	public function GetNumChars($s) {
		if ($this->isUnicodeFont()) {
			return count(TFON::UTF8StringToArray($s, $this->isunicode, $this->CurrentFont));
		}
		return strlen($s);
	}
	...
	protected function getFontsList() {
		if (($fontsdir = opendir(TFON::_getfontpath())) !== false) {
	...
	public function AddFont($family, $style='', $fontfile='', $subset='default') {
		...
		if (TSTA::empty_string($family)) {
			if (!TSTA::empty_string($this->FontFamily)) {
		...
		$fontdir = false;
		if (!TSTA::empty_string($fontfile)) {
			$fontdir = dirname($fontfile);
			if (TSTA::empty_string($fontdir) OR ($fontdir == '.')) {
				$fontdir = '';
			} else {
				$fontdir .= '/';
			}
		...
		if (TSTA::empty_string($fontfile) OR (!@file_exists($fontfile))) {
			// build a standard filenames for specified font
			$tmp_fontfile = str_replace(' ', '', $family).strtolower($style).'.php';
			$fontfile = TFON::getFontFullPath($tmp_fontfile, $fontdir);
			if (TSTA::empty_string($fontfile)) {
				$missing_style = true;
		...
		// include font file
		if (!TSTA::empty_string($fontfile) AND (@file_exists($fontfile))) {
			include($fontfile);
		} else {
		...
		if (!isset($file) OR TSTA::empty_string($file)) {
			$file = '';
		}
		if (!isset($enc) OR TSTA::empty_string($enc)) {
			$enc = '';
		}
		if (!isset($cidinfo) OR TSTA::empty_string($cidinfo)) {
			$cidinfo = array('Registry'=>'Adobe', 'Ordering'=>'Identity', 'Supplement'=>0);
			$cidinfo['uni2cid'] = array();
		}
		if (!isset($ctg) OR TSTA::empty_string($ctg)) {
			$ctg = '';
		}
		if (!isset($desc) OR TSTA::empty_string($desc)) {
			$desc = array();
		}
		if (!isset($up) OR TSTA::empty_string($up)) {
			$up = -100;
		}
		if (!isset($ut) OR TSTA::empty_string($ut)) {
			$ut = 50;
		}
		if (!isset($cw) OR TSTA::empty_string($cw)) {
			$cw = array();
		}
		if (!isset($dw) OR TSTA::empty_string($dw)) {
			// set default width
	...
	public function isCharDefined($char, $font='', $style='') {
		if (is_string($char)) {
			// get character code
			$char = TFON::UTF8StringToArray($char, $this->isunicode, $this->CurrentFont);
			$char = $char[0];
		}
		if (TSTA::empty_string($font)) {
			if (TSTA::empty_string($style)) {
	...
	public function replaceMissingChars($text, $font='', $style='', $subs=array()) {
		...
		$fontdata = $this->AddFont($font, $style);
		$fontinfo = $this->getFontBuffer($fontdata['fontkey']);
		$uniarr = TFON::UTF8StringToArray($text, $this->isunicode, $this->CurrentFont);
		...
		return TFON::UniArrSubString(TFON::UTF8ArrayToUniArray($uniarr, $this->isunicode));
	}
	...
	public function Annotation($x, $y, $w, $h, $text, $opt=array('Subtype'=>'Text'), $spaces=0) {
		...
		if (!$this->pdfa_mode) {
			if ((($opt['Subtype'] == 'FileAttachment') OR ($opt['Subtype'] == 'Sound')) AND (!TSTA::empty_string($opt['FS']))
				AND (@file_exists($opt['FS']) OR TSTA::isValidURL($opt['FS']))
	...
	protected function _putEmbeddedFiles() {
		...
		foreach ($this->embeddedfiles as $filename => $filedata) {
			$data = TSTA::fileGetContents($filedata['file']);
			if ($data !== FALSE) {
				$rawsize = strlen($data);
				if ($rawsize > 0) {
					// update name tree
	...
	protected function getCellCode($w, $h=0, $txt='', $border=0, $ln=0, $align='', $fill=false, $link='', $stretch=0, $ignore_min_height=false, $calign='T', $valign='M') {
		...
		$txt = str_replace(TFON::unichr(160, $this->isunicode), ' ', $txt);
		$prev_cell_margin = $this->cell_margin;
		$prev_cell_padding = $this->cell_padding;
		$txt = TSTA::removeSHY($txt, $this->isunicode);
		...
		if ($txt != '') {
			...
			if ($this->isunicode) {
				if (($this->CurrentFont['type'] == 'core') OR ($this->CurrentFont['type'] == 'TrueType') OR ($this->CurrentFont['type'] == 'Type1')) {
					$txt2 = TFON::UTF8ToLatin1($txt2, $this->isunicode, $this->CurrentFont);
				} else {
					$unicode = TFON::UTF8StringToArray($txt, $this->isunicode, $this->CurrentFont); // array of UTF-8 unicode values
					$unicode = TFON::utf8Bidi($unicode, '', $this->tmprtl, $this->isunicode, $this->CurrentFont);
					...
					$txt2 = TFON::arrUTF8ToUTF16BE($unicode, false);
				}
			}
			$txt2 = TSTA::_escape($txt2);
			...
			if (isset($uniblock)) {
				...
				foreach ($uniblock as $uk => $uniarr) {
					if (($uk % 2) == 0) {
						...
						// character to print
						$topchr = TFON::arrUTF8ToUTF16BE($uniarr, false);
						$topchr = TSTA::_escape($topchr);
	...
	public function MultiCell($w, $h, $txt, $border=0, $align='J', $fill=false, $ln=1, $x='', $y='', $reseth=true, $stretch=0, $ishtml=false, $autopadding=true, $maxh=0, $valign='T', $fitcell=false) {
		...
		$this->setCellMargins(0, 0, 0, 0);
		if (TSTA::empty_string($this->lasth) OR $reseth) {
			// reset row height
			$this->resetLastH();
		}
		if (!TSTA::empty_string($y)) {
		...
		// get border modes
		$border_start = TSTA::getBorderMode($border, $position='start', $this->opencell);
		$border_end = TSTA::getBorderMode($border, $position='end', $this->opencell);
		$border_middle = TSTA::getBorderMode($border, $position='middle', $this->opencell);
	...
	public function getNumLines($txt, $w=0, $reseth=false, $autopadding=true, $cellpadding='', $border=0) {
		...
		$lines = 1;
		$sum = 0;
		$chars = TFON::utf8Bidi(TFON::UTF8StringToArray($txt, $this->isunicode, $this->CurrentFont), $txt, $this->tmprtl, $this->isunicode, $this->CurrentFont);
		...
		for ($i = 0; $i < $length; ++$i) {
			...
			if (($c != 160)
					AND (($c == 173)
						OR preg_match($this->re_spaces, TFON::unichr($c, $this->isunicode))
						OR (($c == 45)
							AND ($i > 0) AND ($i < ($length - 1))
							AND @preg_match('/[\p{L}]/'.$this->re_space['m'], TFON::unichr($chars[($i - 1)], $this->isunicode))
							AND @preg_match('/[\p{L}]/'.$this->re_space['m'], TFON::unichr($chars[($i + 1)], $this->isunicode))
						)
					)
				) {
	...
	public function Write($h, $txt, $link='', $fill=false, $align='', $ln=false, $stretch=0, $firstline=false, $firstblock=false, $maxh=0, $wadj=0, $margin='') {
		...
		if (preg_match(TFOND::$uni_RE_PATTERN_ARABIC, $s)) {
			$arabic = true;
		} else {
			$arabic = false;
		}
		// check if string contains RTL text
		if ($arabic OR ($this->tmprtl == 'R') OR preg_match(TFOND::$uni_RE_PATTERN_RTL, $s)) {
			$rtlmode = true;
		} else {
			$rtlmode = false;
		}
		...
		$chars = TFON::UTF8StringToArray($s, $this->isunicode, $this->CurrentFont);
		// calculate maximum width for a single character on string
		$chrw = $this->GetArrStringWidth($chars, '', '', 0, true);
		array_walk($chrw, array($this, 'getRawCharWidth'));
		$maxchwidth = max($chrw);
		// get array of chars
		$uchars = TFON::UTF8ArrayToUniArray($chars, $this->isunicode);
		// get the number of characters
		$nb = count($chars);
		// replacement for SHY character (minus symbol)
		$shy_replacement = 45;
		$shy_replacement_char = TFON::unichr($shy_replacement, $this->isunicode);
		...
		while ($i < $nb) {
			...
			if ($c == 10) { // 10 = "\n" = new line
				...
				$tmpstr = TFON::UniArrSubString($uchars, $j, $i);
				if ($firstline) {
					$startx = $this->x;
					$tmparr = array_slice($chars, $j, ($i - $j));
					if ($rtlmode) {
						$tmparr = TFON::utf8Bidi($tmparr, $tmpstr, $this->tmprtl, $this->isunicode, $this->CurrentFont);
					}
				...
				if ($firstline) {
					...
					return (TFON::UniArrSubString($uchars, $i));
				}
				++$nl;
				$j = $i + 1;
				$l = 0;
				...
				if (($c != 160)
					AND (($c == 173)
						...
						OR (($c == 45)
							AND ($i < ($nb - 1))
							AND @preg_match('/[\p{L}]/'.$this->re_space['m'], TFON::unichr($pc, $this->isunicode))
							AND @preg_match('/[\p{L}]/'.$this->re_space['m'], TFON::unichr($chars[($i + 1)], $this->isunicode))
						)
					)
				) {
				...
				if ($this->isUnicodeFont() AND ($arabic)) {
					...
					// *** very slow ***
					$l = $this->GetArrStringWidth(TFON::utf8Bidi(array_slice($chars, $j, ($i - $j)), '', $this->tmprtl, $this->isunicode, $this->CurrentFont));
				} else {
				...
				if (($l > $wmax) OR (($c == 173) AND (($l + $tmp_shy_replacement_width) >= $wmax))) {
					...
					if ($sep == -1) {
						...
						if (($this->rtl AND ($this->x <= ($this->w - $this->rMargin - $this->cell_padding['R'] - $margin['R'] - $chrwidth)))
							...
							$linebreak = true;
							if ($firstline) {
								return (TFON::UniArrSubString($uchars, $j));
							}
						} else {
						...
						$strrest = TFON::UniArrSubString($uchars, ($sep + $endspace));
						$nextstr = TSTA::pregSplit('/'.$this->re_space['p'].'/', $this->re_space['m'], $this->stringTrim($strrest));
						if (isset($nextstr[0]) AND ($this->GetStringWidth($nextstr[0]) > $pw)) {
							...
							$tmpstr = TFON::UniArrSubString($uchars, $j, ($sep + $endspace));
							if ($firstline) {
								$startx = $this->x;
							...
							if ($firstline) {
								...
								$this->cell_padding = $tmpcellpadding;
								return (TFON::UniArrSubString($uchars, ($sep + $endspace)));
							}
							$i = $sep;
							$sep = -1;
							$shy = false;
		...
		if ($l > 0) {
			...
			$tmpstr = TFON::UniArrSubString($uchars, $j, $nb);
			if ($firstline) {
				$startx = $this->x;
			...
			if ($firstline) {
				$this->cell_padding = $tmpcellpadding;
				return (TFON::UniArrSubString($uchars, $nb));
			}
			++$nl;
		}
		if ($firstline) {
	...
	public function Image($file, $x='', $y='', $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=300, $palign='', $ismask=false, $imgmask=false, $border=0, $fitbox=false, $hidden=false, $fitonpage=false, $alt=false, $altimgs=array()) {
		...
		if ($file[0] === '@') {
			...
			if (($imsize = @getimagesize($file)) === FALSE) {
				if (in_array($file, $this->imagekeys)) {
					...
					$imgdata = TSTA::fileGetContents($file);
				}
			}
		}
		if (!empty($imgdata)) {
			// copy image to cache
			$original_file = $file;
			$file = TSTA::getObjFilename('img', $this->file_id);
			$fp = TSTA::fopenLocal($file, 'w');
			if (!$fp) {
		...
		if ($newimage) {
			...
			if ($type == '') {
				$type = TIMA::getImageFileType($file, $imsize);
			} elseif ($type == 'jpg') {
				$type = 'jpeg';
			}
			$mqr = TSTA::get_mqr();
			TSTA::set_mqr(false);
			// Specific image handlers (defined on TIMA CLASS)
			$mtd = '_parse'.$type;
			// GD image handler function
			$gdfunction = 'imagecreatefrom'.$type;
			$info = false;
			if ((method_exists('TIMA', $mtd)) AND (!($resize AND (function_exists($gdfunction) OR extension_loaded('imagick'))))) {
					...
					if ($img !== false) {
						if ($resize) {
							...
							if (($type == 'gif') OR ($type == 'png')) {
								$imgr = TIMA::setGDImageTransparency($imgr, $img);
							}
							imagecopyresampled($imgr, $img, 0, 0, 0, 0, $neww, $newh, $pixw, $pixh);
							$img = $imgr;
						}
						if (($type == 'gif') OR ($type == 'png')) {
							$info = TIMA::_toPNG($img, TSTA::getObjFilename('img', $this->file_id));
						} else {
							$info = TIMA::_toJPEG($img, $this->jpeg_quality, TSTA::getObjFilename('img', $this->file_id));
						}
					}
				} catch(Exception $e) {
					$info = false;
				...
				try {
					...
					if ($type == 'svg') {
						if ($file[0] === '@') {
							...
							$svgimg = TSTA::fileGetContents($file);
						}
						if ($svgimg !== FALSE) {
					...
					$tempname = TSTA::getObjFilename('img', $this->file_id);
					$img->writeImage($tempname);
					$info = TIMA::_parsejpeg($tempname);
					unlink($tempname);
					$img->destroy();
				} catch(Exception $e) {
					$info = false;
	...
	public function Output($name='doc.pdf', $dest='I') {
		...
		if ($this->sign) {
			...
			// remove filler space
			$byterange_string_len = strlen(TSTA::$byterange_string);
			// define the ByteRange
			$byte_range = array();
			$byte_range[0] = 0;
			$byte_range[1] = strpos($pdfdoc, TSTA::$byterange_string) + $byterange_string_len + 10;
			...
			$pdfdoc = str_replace(TSTA::$byterange_string, $byterange, $pdfdoc);
			// write the document to a temporary folder
			$tempdoc = TSTA::getObjFilename('doc', $this->file_id);
			$f = TSTA::fopenLocal($tempdoc, 'wb');
			if (!$f) {
			...
			$tempsign = TSTA::getObjFilename('sig', $this->file_id);
		...
		switch($dest) {
			case 'I': {
				...
				if (php_sapi_name() != 'cli') {
					...
					TSTA::sendOutputData($this->getBuffer(), $this->bufferlen);
				} else {
					echo $this->getBuffer();
				}
				break;
			...
			case 'FD': {
				...
				if ($dest == 'FI') {
					...
					header('Last-Modified: '.gmdate('D, d M Y H:i:s').' GMT');
					header('Content-Disposition: inline; filename="'.basename($name).'"');
					TSTA::sendOutputData(file_get_contents($name), filesize($name));
				} elseif ($dest == 'FD') {
					// send headers to browser
					if (ob_get_contents()) {
						$this->Error('Some data has already been output, can\'t send PDF file');
	...
	protected function getInternalPageNumberAliases($a= '') {
		...
		$u = '{'.$a.'}';
		$alias['u'][] = TSTA::_escape($u);
		if ($this->isunicode) {
			$alias['u'][] = TSTA::_escape(TFON::UTF8ToLatin1($u, $this->isunicode, $this->CurrentFont));
			$alias['u'][] = TSTA::_escape(TFON::utf8StrRev($u, false, $this->tmprtl, $this->isunicode, $this->CurrentFont));
			$alias['a'][] = TSTA::_escape(TFON::UTF8ToLatin1($a, $this->isunicode, $this->CurrentFont));
			$alias['a'][] = TSTA::_escape(TFON::utf8StrRev($a, false, $this->tmprtl, $this->isunicode, $this->CurrentFont));
		}
		$alias['a'][] = TSTA::_escape($a);
		return $alias;
	}
	...
	protected function getAllInternalPageNumberAliases() {
		$basic_alias = array(TSTA::$alias_tot_pages, TSTA::$alias_num_page, TSTA::$alias_group_tot_pages, TSTA::$alias_group_num_page, TSTA::$alias_right_shift);
		$pnalias = array();
			...
			foreach ($alias as $a) {
				...
				if (($pos = strpos($page, $a)) !== false) {
					...
					if ($type == 'u') {
						...
						$shift = TFON::UTF8ToUTF16BE($shift, false, $this->isunicode, $this->CurrentFont);
					} else {
	...
	protected function setPageBoxTypes($boxes) {
		...
		foreach ($boxes as $box) {
			if (in_array($box, TSTA::$pageboxes)) {
				$this->page_boxes[] = $box;
			}
		}
	}
	...
	protected function _putpages() {
		...
		$ptpa = TSTA::formatPageNumber(($this->starting_page_number + $num_pages - 1));
		$ptpu = TFON::UTF8ToUTF16BE($ptpa, false, $this->isunicode, $this->CurrentFont);
		...
		for ($n = 1; $n <= $num_pages; ++$n) {
			...
			$pnpa = TSTA::formatPageNumber(($this->starting_page_number + $n - 1));
			$pnpu = TFON::UTF8ToUTF16BE($pnpa, false, $this->isunicode, $this->CurrentFont);
			...
			if (!empty($this->pagegroups)) {
				if (isset($this->newpagegroup[$n])) {
					$pagegroupnum = 0;
					++$groupnum;
					$ptga = TSTA::formatPageNumber($this->pagegroups[$groupnum]);
					$ptgu = TFON::UTF8ToUTF16BE($ptga, false, $this->isunicode, $this->CurrentFont);
					$ptg_num_chars = $this->GetNumChars($ptga);
				}
				++$pagegroupnum;
				$pnga = TSTA::formatPageNumber($pagegroupnum);
				$pngu = TFON::UTF8ToUTF16BE($pnga, false, $this->isunicode, $this->CurrentFont);
				...
				list($temppage, $gdiff) = TSTA::replacePageNumAliases($temppage, $replace, $gdiff);
			}
			// replace page numbers
			$replace = array();
			...
			list($temppage, $pdiff) = TSTA::replacePageNumAliases($temppage, $replace, $pdiff);
			...
			if (isset($this->PageAnnots[$n])) {
				...
				foreach ($this->PageAnnots[$n] as $key => $pl) {
					...
					if (isset($pl['opt']['c']) AND (is_array($pl['opt']['c'])) AND !empty($pl['opt']['c'])) {
						$annots .= ' /C '.TCOL::getColorStringFromArray($pl['opt']['c']);
					}
					//$annots .= ' /StructParent ';
					//$annots .= ' /OC ';
						...
						case 'link': {
							if (is_string($pl['txt']) && !empty($pl['txt'])) {
								if ($pl['txt'][0] == '#') {
									...
									$annots .= ' /Dest /'.TSTA::encodeNameObject(substr($pl['txt'], 1));
								} elseif ($pl['txt'][0] == '%') {
									// embedded PDF file
						...
						case 'widget': {
							...
							if (isset($pl['opt']['mk']) AND (is_array($pl['opt']['mk'])) AND !empty($pl['opt']['mk'])) {
								...
								if (isset($pl['opt']['mk']['bc']) AND (is_array($pl['opt']['mk']['bc']))) {
									$annots .= ' /BC '.TCOL::getColorStringFromArray($pl['opt']['mk']['bc']);
								}
								if (isset($pl['opt']['mk']['bg']) AND (is_array($pl['opt']['mk']['bg']))) {
									$annots .= ' /BG '.TCOL::getColorStringFromArray($pl['opt']['mk']['bg']);
								}
	...
	protected function _putfonts() {
		...
		foreach ($this->FontFiles as $file => $info) {
			...
			$fontfile = TFON::getFontFullPath($file, $info['fontdir']);
			if (!TSTA::empty_string($fontfile)) {
				...
				if ((!$compressed) AND (isset($info['length2']))) {
					...
					$font = TFON::_getTrueTypeFontSubset($font, $subsetchars);
		...
		foreach ($this->fontkeys as $k) {
			...
			if ($type == 'core') {
				...
				if (!TSTA::empty_string($font['file'])) {
	...
	protected function _puttruetypeunicode($font) {
		...
		$out .= "\n".TFON::_putfontwidths($font, 0);
		if (isset($font['ctg']) AND (!TSTA::empty_string($font['ctg']))) {
		...
		if (isset($font['ctg']) AND (!TSTA::empty_string($font['ctg']))) {
			...
			// search and get ctg font file to embedd
			$fontfile = TFON::getFontFullPath($ctgfile, $fontdir);
			if (TSTA::empty_string($fontfile)) {
	...
	protected function _putcidfont0($font) {
		...
		$out .= ' /FontDescriptor '.($this->n + 1).' 0 R';
		$out .= ' /DW '.$font['dw'];
		$out .= "\n".TFON::_putfontwidths($font, $cidoffset);
		$out .= ' >>';
		$out .= "\n".'endobj';
		$this->_out($out);
		$this->_newobj();
	...
	protected function _putinfo() {
		...
		if (!TSTA::empty_string($this->title)) {
			// The document's title.
			$out .= ' /Title '.$this->_textstring($this->title, $oid);
		}
		if (!TSTA::empty_string($this->author)) {
			// The name of the person who created the document.
			$out .= ' /Author '.$this->_textstring($this->author, $oid);
		}
		if (!TSTA::empty_string($this->subject)) {
			// The subject of the document.
			$out .= ' /Subject '.$this->_textstring($this->subject, $oid);
		}
		if (!TSTA::empty_string($this->keywords)) {
			// Keywords associated with the document.
			$out .= ' /Keywords '.$this->_textstring($this->keywords, $oid);
		}
		if (!TSTA::empty_string($this->creator)) {
		...
		// default producer
		$out .= ' /Producer '.$this->_textstring(TSTA::getTCPDFProducer(), $oid);
	...
	protected function _putXMP() {
		...
		$this->encrypted = false;
		// set XMP data
		$xmp = '<?xpacket begin="'.TFON::unichr(0xfeff, $this->isunicode).'" id="W5M0MpCehiHzreSzNTczkc9d"?>'."\n";
		...
		$xmp .= "\t\t\t".'<dc:title>'."\n";
		$xmp .= "\t\t\t\t".'<rdf:Alt>'."\n";
		$xmp .= "\t\t\t\t\t".'<rdf:li xml:lang="x-default">'.TSTA::_escapeXML($this->title).'</rdf:li>'."\n";
		$xmp .= "\t\t\t\t".'</rdf:Alt>'."\n";
		$xmp .= "\t\t\t".'</dc:title>'."\n";
		$xmp .= "\t\t\t".'<dc:creator>'."\n";
		$xmp .= "\t\t\t\t".'<rdf:Seq>'."\n";
		$xmp .= "\t\t\t\t\t".'<rdf:li>'.TSTA::_escapeXML($this->author).'</rdf:li>'."\n";
		$xmp .= "\t\t\t\t".'</rdf:Seq>'."\n";
		$xmp .= "\t\t\t".'</dc:creator>'."\n";
		$xmp .= "\t\t\t".'<dc:description>'."\n";
		$xmp .= "\t\t\t\t".'<rdf:Alt>'."\n";
		$xmp .= "\t\t\t\t\t".'<rdf:li xml:lang="x-default">'.TSTA::_escapeXML($this->subject).'</rdf:li>'."\n";
		$xmp .= "\t\t\t\t".'</rdf:Alt>'."\n";
		$xmp .= "\t\t\t".'</dc:description>'."\n";
		$xmp .= "\t\t\t".'<dc:subject>'."\n";
		$xmp .= "\t\t\t\t".'<rdf:Bag>'."\n";
		$xmp .= "\t\t\t\t\t".'<rdf:li>'.TSTA::_escapeXML($this->keywords).'</rdf:li>'."\n";
		$xmp .= "\t\t\t\t".'</rdf:Bag>'."\n";
		$xmp .= "\t\t\t".'</dc:subject>'."\n";
		$xmp .= "\t\t".'</rdf:Description>'."\n";
		// convert doc creation date format
		$dcdate = TSTA::getFormattedDate($this->doc_creation_timestamp);
		...
		$doccreationdate = TSTA::_escapeXML($doccreationdate);
		// convert doc modification date format
		$dmdate = TSTA::getFormattedDate($this->doc_modification_timestamp);
		...
		$docmoddate = TSTA::_escapeXML($docmoddate);
		...
		$xmp .= "\t\t\t".'<pdf:Keywords>'.TSTA::_escapeXML($this->keywords).'</pdf:Keywords>'."\n";
		$xmp .= "\t\t\t".'<pdf:Producer>'.TSTA::_escapeXML(TSTA::getTCPDFProducer()).'</pdf:Producer>'."\n";
		$xmp .= "\t\t".'</rdf:Description>'."\n";
	...
	protected function _putcatalog() {
		...
		if (isset($this->LayoutMode) AND (!TSTA::empty_string($this->LayoutMode))) {
			$out .= ' /PageLayout /'.$this->LayoutMode;
		}
		if (isset($this->PageMode) AND (!TSTA::empty_string($this->PageMode))) {
	...
	protected function _putviewerpreferences() {
		...
		if (isset($vp['Duplex']) AND (!TSTA::empty_string($vp['Duplex']))) {
	...
	protected function _beginpage($orientation='', $format='') {
		...
		$this->state = 2;
		if (TSTA::empty_string($orientation)) {
			if (isset($this->CurOrientation)) {
	...
	protected function _datestring($n=0, $timestamp=0) {
		...
		return $this->_datastring('D:'.TSTA::getFormattedDate($timestamp), $n);
	}
	...
	protected function _textstring($s, $n=0) {
		if ($this->isunicode) {
			...
			$s = TFON::UTF8ToUTF16BE($s, true, $this->isunicode, $this->CurrentFont);
		}
	...
	protected function _objectkey($n) {
		...
		$objkey = substr(TSTA::_md5_16($objkey), 0, (($this->encryptdata['Length'] / 8) + 5));
	...
	protected function _encrypt_data($n, $s) {
		...
		switch ($this->encryptdata['mode']) {
			case 0:   // RC4-40
			case 1: { // RC4-128
				$s = TSTA::_RC4($this->_objectkey($n), $s, $this->last_enc_key, $this->last_enc_key_c);
				break;
			}
			case 2: { // AES-128
				$s = TSTA::_AES($this->_objectkey($n), $s);
				break;
			}
			case 3: { // AES-256
				$s = TSTA::_AES($this->encryptdata['key'], $s);
				break;
			}
		}
		return $s;
	...
	protected function _putencryption() {
		...
		if ($this->encryptdata['pubkey']) {
			...
			if ($this->encryptdata['V'] == 5) { // AES-256
				$out .= ' 5';
				$out .= ' /OE ('.TSTA::_escape($this->encryptdata['OE']).')';
				$out .= ' /UE ('.TSTA::_escape($this->encryptdata['UE']).')';
				$out .= ' /Perms ('.TSTA::_escape($this->encryptdata['perms']).')';
			...
			$out .= ' /O ('.TSTA::_escape($this->encryptdata['O']).')';
			$out .= ' /U ('.TSTA::_escape($this->encryptdata['U']).')';
	...
	protected function _Uvalue() {
		if ($this->encryptdata['mode'] == 0) { // RC4-40
			return TSTA::_RC4($this->encryptdata['key'], TSTA::$enc_padding, $this->last_enc_key, $this->last_enc_key_c);
		} elseif ($this->encryptdata['mode'] < 3) { // RC4-128, AES-128
			$tmp = TSTA::_md5_16(TSTA::$enc_padding.$this->encryptdata['fileid']);
			$enc = TSTA::_RC4($this->encryptdata['key'], $tmp, $this->last_enc_key, $this->last_enc_key_c);
			$len = strlen($tmp);
			for ($i = 1; $i <= 19; ++$i) {
				$ek = '';
				for ($j = 0; $j < $len; ++$j) {
				...
				$enc = TSTA::_RC4($ek, $enc, $this->last_enc_key, $this->last_enc_key_c);
			}
			$enc .= str_repeat("\x00", 16);
			return substr($enc, 0, 32);
		} elseif ($this->encryptdata['mode'] == 3) { // AES-256
			$seed = TSTA::_md5_16(TSTA::getRandomSeed());
	...
	protected function _UEvalue() {
		...
		return TSTA::_AESnopad($hashkey, $this->encryptdata['key']);
	}
	...
	protected function _Ovalue() {
		if ($this->encryptdata['mode'] < 3) { // RC4-40, RC4-128, AES-128
			$tmp = TSTA::_md5_16($this->encryptdata['owner_password']);
			if ($this->encryptdata['mode'] > 0) {
				for ($i = 0; $i < 50; ++$i) {
					$tmp = TSTA::_md5_16($tmp);
				}
			}
			$owner_key = substr($tmp, 0, ($this->encryptdata['Length'] / 8));
			$enc = TSTA::_RC4($owner_key, $this->encryptdata['user_password'], $this->last_enc_key, $this->last_enc_key_c);
			if ($this->encryptdata['mode'] > 0) {
	...
	protected function _fixAES256Password($password) {
		...
		$psw_array = TFON::utf8Bidi(TFON::UTF8StringToArray($password, $this->isunicode, $this->CurrentFont), $password, $this->rtl, $this->isunicode, $this->CurrentFont);
		foreach ($psw_array as $c) {
			$psw .= TFON::unichr($c, $this->isunicode);
		}
	...
	protected function _generateencryptionkey() {
		...
		if (!$this->encryptdata['pubkey']) { // standard mode
			if ($this->encryptdata['mode'] == 3) { // AES-256
				...
				$this->encryptdata['key'] = substr(hash('sha256', TSTA::getRandomSeed(), true), 0, $keybytelen);
				// truncate passwords
				...
				$perms = TSTA::getEncPermissionsString($this->encryptdata['protection']); // bytes 0-3
				...
				$this->encryptdata['perms'] = TSTA::_AESnopad($this->encryptdata['key'], $perms);
			} else { // RC4-40, RC4-128, AES-128
				// Pad passwords
				$this->encryptdata['user_password'] = substr($this->encryptdata['user_password'].TSTA::$enc_padding, 0, 32);
				$this->encryptdata['owner_password'] = substr($this->encryptdata['owner_password'].TSTA::$enc_padding, 0, 32);
				// Compute O value
				$this->encryptdata['O'] = $this->_Ovalue();
				// get default permissions (reverse byte order)
				$permissions = TSTA::getEncPermissionsString($this->encryptdata['protection']);
				// Compute encryption key
				$tmp = TSTA::_md5_16($this->encryptdata['user_password'].$this->encryptdata['O'].$permissions.$this->encryptdata['fileid']);
				if ($this->encryptdata['mode'] > 0) {
					for ($i = 0; $i < 50; ++$i) {
						$tmp = TSTA::_md5_16(substr($tmp, 0, $keybytelen));
					}
				}
			...
			// random 20-byte seed
			$seed = sha1(TSTA::getRandomSeed(), true);
			$recipient_bytes = '';
			foreach ($this->encryptdata['pubkeys'] as $pubkey) {
				// for each public certificate
				if (isset($pubkey['p'])) {
					$pkprotection = TSTA::getUserPermissionCode($pubkey['p'], $this->encryptdata['mode']);
				} else {
					$pkprotection = $this->encryptdata['protection'];
				}
				// get default permissions (reverse byte order)
				$pkpermissions = TSTA::getEncPermissionsString($pkprotection);
				// envelope data
				$envelope = $seed.$pkpermissions;
				// write the envelope data to a temporary file
				$tempkeyfile = TSTA::getObjFilename('key', $this->file_id);
				$f = TSTA::fopenLocal($tempkeyfile, 'wb');
				if (!$f) {
				...
				fclose($f);
				$tempencfile = TSTA::getObjFilename('enc', $this->file_id);
	...
	public function SetProtection($permissions=array('print', 'modify', 'copy', 'annot-forms', 'fill-forms', 'extract', 'assemble', 'print-high'), $user_pass='', $owner_pass=null, $mode=0, $pubkeys=null) {
		...
		$this->encryptdata['protection'] = TSTA::getUserPermissionCode($permissions, $mode);
		...
		if ($owner_pass === null) {
			$owner_pass = md5(TSTA::getRandomSeed());
		}
		...
		$this->encrypted = true;
		$this->encryptdata['fileid'] = TSTA::convertHexStringToString($this->file_id);
	...
	public function Rect($x, $y, $w, $h, $style='', $border_style=array(), $fill_color=array()) {
		...
		if (!empty($style)) {
			$op = TSTA::getPathPaintOperator($style);
	...
	public function Ellipse($x0, $y0, $rx, $ry='', $angle=0, $astart=0, $afinish=360, $style='', $line_style=array(), $fill_color=array(), $nc=2) {
		...
		if (TSTA::empty_string($ry) OR ($ry == 0)) {
			$ry = $rx;
		}
		if (!(false === strpos($style, 'F')) AND isset($fill_color)) {
			$this->SetFillColorArray($fill_color);
	...
	public function setDestination($name, $y=-1, $page='', $x=-1) {
		...
		$name = TSTA::encodeNameObject($name);
		if (TSTA::empty_string($name)) {
			return false;
		}
		if ($y == -1) {
			$y = $this->GetY();
		...
		foreach ($this->outlines as $i => $o) {
			...
			if (isset($o['u']) AND !empty($o['u'])) {
				...
				if (is_string($o['u'])) {
					if ($o['u'][0] == '#') {
						// internal destination
						$out .= ' /Dest /'.TSTA::encodeNameObject(substr($o['u'], 1));
					} elseif ($o['u'][0] == '%') {
						// embedded PDF file
	...
	public function TextField($name, $w, $h, $prop=array(), $opt=array(), $x='', $y='', $js=false) {
		...
		// get annotation data
		$popt = TSTA::getAnnotOptFromJSProp($prop, $this->spot_colors, $this->rtl);
	...
	protected function _putsignature() {
		...
		if (empty($this->signature_data['approval']) OR ($this->signature_data['approval'] != 'A')) {
			...
			if ($this->signature_data['cert_type'] > 0) {
				...
				$out .= ' /Type /TransformParams';
				$out .= ' /V /2.2';
				if (!TSTA::empty_string($this->ur['document'])) {
					$out .= ' /Document['.$this->ur['document'].']';
				}
				if (!TSTA::empty_string($this->ur['form'])) {
					$out .= ' /Form['.$this->ur['form'].']';
				}
				if (!TSTA::empty_string($this->ur['signature'])) {
					$out .= ' /Signature['.$this->ur['signature'].']';
				}
				if (!TSTA::empty_string($this->ur['annots'])) {
					$out .= ' /Annots['.$this->ur['annots'].']';
				}
				if (!TSTA::empty_string($this->ur['ef'])) {
					$out .= ' /EF['.$this->ur['ef'].']';
				}
				if (!TSTA::empty_string($this->ur['formex'])) {
		...
		if (isset($this->signature_data['info']['Name']) AND !TSTA::empty_string($this->signature_data['info']['Name'])) {
			$out .= ' /Name '.$this->_textstring($this->signature_data['info']['Name'], $sigobjid);
		}
		if (isset($this->signature_data['info']['Location']) AND !TSTA::empty_string($this->signature_data['info']['Location'])) {
			$out .= ' /Location '.$this->_textstring($this->signature_data['info']['Location'], $sigobjid);
		}
		if (isset($this->signature_data['info']['Reason']) AND !TSTA::empty_string($this->signature_data['info']['Reason'])) {
			$out .= ' /Reason '.$this->_textstring($this->signature_data['info']['Reason'], $sigobjid);
		}
		if (isset($this->signature_data['info']['ContactInfo']) AND !TSTA::empty_string($this->signature_data['info']['ContactInfo'])) {
	...
	public function getAliasRightShift() {
		...
		$ref = '{'.TSTA::$alias_right_shift.'}{'.TSTA::$alias_tot_pages.'}{'.TSTA::$alias_num_page.'}';
		...
		$alias = TSTA::$alias_right_shift.$sdiff.'}';
		if ($this->isUnicodeFont()) {
			$alias = '{'.$alias;
		}
		return $alias;
	...
	public function getPageGroupAlias() {
		if ($this->isUnicodeFont()) {
			return '{'.TSTA::$alias_group_tot_pages.'}';
		}
		return TSTA::$alias_group_tot_pages;
	}
	...
	public function getPageNumGroupAlias() {
		if ($this->isUnicodeFont()) {
			return '{'.TSTA::$alias_group_num_page.'}';
		}
		return TSTA::$alias_group_num_page;
	}
	...
	public function getGroupPageNoFormatted() {
		return TSTA::formatPageNumber($this->getGroupPageNo());
	}
	...
	public function PageNoFormatted() {
		return TSTA::formatPageNumber($this->PageNo());
	}
	...
	public function setOverprint($stroking=true, $nonstroking='', $mode=0) {
		...
		if (TSTA::empty_string($nonstroking)) {
		...
		foreach ($bars as $col) {
			switch ($col) {
				...
				default: { // SPECIFIC SPOT COLOR NAME
					$col_a = array(0,0,0,0,'None');
					$col_b = TCOL::getSpotColor($col, $this->spot_colors);
					if ($col_b === false) {
	...
	public function ImageEps($file, $x='', $y='', $w=0, $h=0, $link='', $useBoundingBox=true, $align='', $palign='', $border=0, $fitonpage=false, $fixoutvals=false) {
		...
		if ($file[0] === '@') { // image from string
			...
			$data = TSTA::fileGetContents($file);
		}
		if ($data === FALSE) {
	...
	public function write1DBarcode($code, $type, $x='', $y='', $w='', $h='', $xres='', $style='', $align='') {
		if (TSTA::empty_string(trim($code))) {
			return;
		}
		...
		// print text
		if ($style['text']) {
			if (isset($style['label']) AND !TSTA::empty_string($style['label'])) {
				$label = $style['label'];
			} else {
				$label = $code;
			}
	...
	public function fixHTMLCode($html, $default_css='', $tagvs='', $tidy_options='') {
		return TSTA::fixHTMLCode($html, $default_css, $tagvs, $tidy_options, $this->tagvspaces);
	}
	...
	protected function getCSSBorderStyle($cssborder) {
		...
		$border['color'] = TCOL::convertHTMLColorToDec($color, $this->spot_colors);
		return $border;
	}
	...
	public function getHTMLFontUnits($val, $refsize=12, $parent_size=12, $defaultunit='pt') {
		$refsize = TFON::getFontRefSize($refsize);
		$parent_size = TFON::getFontRefSize($parent_size, $refsize);
		switch ($val) {
			case 'xx-small': {
				$size = ($refsize - 4);
				break;
				...
				if (preg_match('/type[\s]*=[\s]*"text\/css"/', $link, $type)) {
					...
					if (empty($type) OR (isset($type[1]) AND (($type[1] == 'all') OR ($type[1] == 'print')))) {
						...
						if (preg_match('/href[\s]*=[\s]*"([^"]*)"/', $link, $type) > 0) {
							// read CSS data file
							$cssdata = TSTA::fileGetContents(trim($type[1]));
							if (($cssdata !== FALSE) AND (strlen($cssdata) > 0)) {
								$css = array_merge($css, TSTA::extractCSSproperties($cssdata));
							}
						}
					}
				}
			...
			if (preg_match($tagpattern, $element)) {
				...
				if ($element[0] == '/') {
					...
					if (($dom[$key]['value'] == 'tr') AND ($dom[($dom[$key]['parent'])]['thead'] === true)) {
						if (TSTA::empty_string($dom[($dom[($dom[$key]['parent'])]['parent'])]['thead'])) {
					...
					if (($dom[$key]['value'] == 'table') AND (!TSTA::empty_string($dom[($dom[$key]['parent'])]['thead']))) {
					...
					if (!empty($css)) {
						...
						list($dom[$key]['csssel'], $dom[$key]['cssdata']) = TSTA::getCSSdataArray($dom, $key, $css);
						$dom[$key]['attribute']['style'] = TSTA::getTagStyleFromCSSarray($dom[$key]['cssdata']);
					}
					...
					if (isset($dom[$key]['attribute']['style']) AND !empty($dom[$key]['attribute']['style'])) {
						...
						// font color
						if (isset($dom[$key]['style']['color']) AND (!TSTA::empty_string($dom[$key]['style']['color']))) {
							$dom[$key]['fgcolor'] = TCOL::convertHTMLColorToDec($dom[$key]['style']['color'], $this->spot_colors);
						} elseif ($dom[$key]['value'] == 'a') {
							$dom[$key]['fgcolor'] = $this->htmlLinkColorArray;
						}
						// background color
						if (isset($dom[$key]['style']['background-color']) AND (!TSTA::empty_string($dom[$key]['style']['background-color']))) {
							$dom[$key]['bgcolor'] = TCOL::convertHTMLColorToDec($dom[$key]['style']['background-color'], $this->spot_colors);
						}
						// text-decoration
						...
						if (isset($dom[$key]['style']['border-color'])) {
							...
							if (isset($brd_colors[3])) {
								$dom[$key]['border']['L']['color'] = TCOL::convertHTMLColorToDec($brd_colors[3], $this->spot_colors);
							}
							if (isset($brd_colors[1])) {
								$dom[$key]['border']['R']['color'] = TCOL::convertHTMLColorToDec($brd_colors[1], $this->spot_colors);
							}
							if (isset($brd_colors[0])) {
								$dom[$key]['border']['T']['color'] = TCOL::convertHTMLColorToDec($brd_colors[0], $this->spot_colors);
							}
							if (isset($brd_colors[2])) {
								$dom[$key]['border']['B']['color'] = TCOL::convertHTMLColorToDec($brd_colors[2], $this->spot_colors);
							}
						}
						...
						foreach ($cellside as $bsk => $bsv) {
							...
							if (isset($dom[$key]['style']['border-'.$bsv.'-color'])) {
								$dom[$key]['border'][$bsk]['color'] = TCOL::convertHTMLColorToDec($dom[$key]['style']['border-'.$bsv.'-color'], $this->spot_colors);
							}
					...
					if ((($dom[$key]['value'] == 'ul') OR ($dom[$key]['value'] == 'ol') OR ($dom[$key]['value'] == 'dl'))
						AND (!isset($dom[$key]['align']) OR TSTA::empty_string($dom[$key]['align']) OR ($dom[$key]['align'] != 'J'))) {
						if ($this->rtl) {
							$dom[$key]['align'] = 'R';
						} else {
							$dom[$key]['align'] = 'L';
					...
					if (isset($dom[$key]['attribute']['color']) AND (!TSTA::empty_string($dom[$key]['attribute']['color']))) {
						$dom[$key]['fgcolor'] = TCOL::convertHTMLColorToDec($dom[$key]['attribute']['color'], $this->spot_colors);
					} elseif (!isset($dom[$key]['style']['color']) AND ($dom[$key]['value'] == 'a')) {
						$dom[$key]['fgcolor'] = $this->htmlLinkColorArray;
					}
					// set background color attribute
					if (isset($dom[$key]['attribute']['bgcolor']) AND (!TSTA::empty_string($dom[$key]['attribute']['bgcolor']))) {
						$dom[$key]['bgcolor'] = TCOL::convertHTMLColorToDec($dom[$key]['attribute']['bgcolor'], $this->spot_colors);
					}
					// set stroke color attribute
					if (isset($dom[$key]['attribute']['strokecolor']) AND (!TSTA::empty_string($dom[$key]['attribute']['strokecolor']))) {
						$dom[$key]['strokecolor'] = TCOL::convertHTMLColorToDec($dom[$key]['attribute']['strokecolor'], $this->spot_colors);
					}
					// check for width attribute
					...
					// check for text alignment
					if (isset($dom[$key]['attribute']['align']) AND (!TSTA::empty_string($dom[$key]['attribute']['align'])) AND ($dom[$key]['value'] !== 'img')) {
...
Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
	...
	public function writeHTML($html, $ln=true, $fill=false, $reseth=false, $cell=false, $align='') {
		...
		while ($key < $maxel) {
			...
			if (($dom[$key]['value'] == 'tr') AND isset($dom[$key]['thead']) AND $dom[$key]['thead']) {
				if (isset($dom[$key]['parent']) AND isset($dom[$dom[$key]['parent']]['thead']) AND !TSTA::empty_string($dom[$dom[$key]['parent']]['thead'])) {
					$this->inthead = true;
				...
				if (isset($startlinex)) {
					...
					if ((isset($plalign) AND ((($plalign == 'C') OR ($plalign == 'J') OR (($plalign == 'R') AND (!$this->rtl)) OR (($plalign == 'L') AND ($this->rtl)))))) {
						...
						if ($this->isRTLTextDir()) { // RTL
							...
							$pos1 = TSTA::revstrpos($pmid, '[(');
							if ($pos1 > 0) {
								$pos1 = intval($pos1);
								if ($this->isUnicodeFont()) {
									$pos2 = intval(TSTA::revstrpos($pmid, '[('.chr(0).chr(32)));
									$spacelen = 2;
								} else {
									$pos2 = intval(TSTA::revstrpos($pmid, '[('.chr(32)));
									$spacelen = 1;
								}
								if ($pos1 == $pos2) {
							...
							if ($pos1 > 0) {
								$pos1 = intval($pos1);
								if ($this->isUnicodeFont()) {
									$pos2 = intval(TSTA::revstrpos($pmid, chr(0).chr(32).')]')) + 2;
									$spacelen = 2;
								} else {
									$pos2 = intval(TSTA::revstrpos($pmid, chr(32).')]')) + 1;
									$spacelen = 1;
								}
								if ($pos1 == $pos2) {
							...
							if (preg_match_all('/\[\(([^\)]*)\)\]/x', $pmidtemp, $lnstring, PREG_PATTERN_ORDER)) {
								...
								while (preg_match('/([0-9\.\+\-]*)[\s](Td|cm|m|l|c|re)[\s]/x', $pmid, $strpiece, PREG_OFFSET_CAPTURE, $offset) == 1) {
									...
									switch ($strpiece[2][0]) {
										...
										case 're': {
											// justify block
											if (!TSTA::empty_string($this->lispacer)) {
												$this->lispacer = '';
												continue;
											}
			...
			if ($dom[$key]['tag']) {
				...
				// print list-item
				if (!TSTA::empty_string($this->lispacer) AND ($this->lispacer != '^')) {
				...
				if (!isset($dom[$key]['trimmed_space']) OR !$dom[$key]['trimmed_space']) {
					...
					if (!empty($this->HREF) AND (isset($this->HREF['url']))) {
						...
						if (($strlinelen < $cwa) AND (isset($dom[($key + 1)])) AND ($dom[($key + 1)]['tag']) AND (!$dom[($key + 1)]['block'])) {
							...
							while ($write_block AND isset($dom[$nkey])) {
								if ($dom[$nkey]['tag']) {
									...
									$nextstr = TSTA::pregSplit('/'.$this->re_space['p'].'+/', $this->re_space['m'], $dom[$nkey]['value']);
						...
						if (($wadj > 0) AND (($strlinelen + $wadj) >= $cwa)) {
							$wadj = 0;
							$nextstr = TSTA::pregSplit('/'.$this->re_space['p'].'/', $this->re_space['m'], $dom[$key]['value']);
		...
		switch($tag['value']) {
			case 'table': {
				...
				if (!isset($dom[$key]['attribute']['nested']) OR ($dom[$key]['attribute']['nested'] != 'true')) {
					$this->htmlvspace = 0;
					// set table header
					if (!TSTA::empty_string($dom[$key]['thead'])) {
						// set table header
			...
			case 'img': {
				if (!empty($tag['attribute']['src'])) {
					if ($tag['attribute']['src'][0] === '@') {
						...
						// get image type
						$type = TIMA::getImageFileType($tag['attribute']['src']);
					}
					if (!isset($tag['width'])) {
						$tag['width'] = 0;
					}
					...
					$xpos = $this->x;
					$imglink = '';
					if (isset($this->HREF['url']) AND !TSTA::empty_string($this->HREF['url'])) {
			...
			case 'li': {
				...
				if ($this->listordered[$this->listnum]) {
					// ordered item
					if (isset($parent['attribute']['type']) AND !TSTA::empty_string($parent['attribute']['type'])) {
						$this->lispacer = $parent['attribute']['type'];
					} elseif (isset($parent['listtype']) AND !TSTA::empty_string($parent['listtype'])) {
						$this->lispacer = $parent['listtype'];
					} elseif (isset($this->lisymbol) AND !TSTA::empty_string($this->lisymbol)) {
			...
			case 'input': {
				if (isset($tag['attribute']['name']) AND !TSTA::empty_string($tag['attribute']['name'])) {
				...
				$prop = array();
				$opt = array();
				if (isset($tag['attribute']['readonly']) AND !TSTA::empty_string($tag['attribute']['readonly'])) {
					$prop['readonly'] = true;
				}
				if (isset($tag['attribute']['value']) AND !TSTA::empty_string($tag['attribute']['value'])) {
					$value = $tag['attribute']['value'];
				}
				if (isset($tag['attribute']['maxlength']) AND !TSTA::empty_string($tag['attribute']['maxlength'])) {
					$opt['maxlen'] = intval($tag['attribute']['maxlength']);
				}
				$h = $this->getCellHeight($this->FontSize);
				if (isset($tag['attribute']['size']) AND !TSTA::empty_string($tag['attribute']['size'])) {
				...
				switch ($tag['attribute']['type']) {
					...
					case 'image': {
						...
						if (isset($tag['attribute']['src']) AND !TSTA::empty_string($tag['attribute']['src'])) {
			...
			case 'textarea': {
				...
				if (isset($tag['attribute']['cols']) AND !TSTA::empty_string($tag['attribute']['cols'])) {
					$w = intval($tag['attribute']['cols']) * $this->GetStringWidth(chr(32)) * 2;
				} else {
					$w = 40;
				}
				if (isset($tag['attribute']['rows']) AND !TSTA::empty_string($tag['attribute']['rows'])) {
			...
			case 'select': {
				...
				$w = 0;
				if (isset($tag['attribute']['opt']) AND !TSTA::empty_string($tag['attribute']['opt'])) {
	...
	protected function drawHTMLTagBorder($tag, $xmax) {
		...
		if (isset($tag['border']) AND !empty($tag['border'])) {
			...
			if (!TSTA::empty_string($this->thead) AND (!$this->inthead)) {
				...
				$border = TSTA::getBorderMode($border, $position='middle', $this->opencell);
			}
		}
	...
	protected function putHtmlListBullet($listdepth, $listtype='', $size=10) {
		...
		switch ($listtype) {
			...
			case 'i':
			case 'lower-roman': {
				$textitem = strtolower(TSTA::intToRoman($this->listcount[$this->listnum]));
				break;
			}
			case 'I':
			case 'upper-roman': {
				$textitem = TSTA::intToRoman($this->listcount[$this->listnum]);
				break;
			}
			case 'a':
			case 'lower-alpha':
			...
			case 'lower-greek': {
				$textitem = TFON::unichr((945 + $this->listcount[$this->listnum] - 1), $this->isunicode);
				break;
			}
			/*
	...
	public function addTOC($page='', $numbersfont='', $filler='.', $toc_name='TOC', $style='', $color=array(0,0,0)) {
		...
		$page_fill_end = false;
		$current_column = $this->current_column;
		if (TSTA::empty_string($numbersfont)) {
			$numbersfont = $this->default_monospaced_font;
		}
		if (TSTA::empty_string($filler)) {
			$filler = ' ';
		}
		if (TSTA::empty_string($page)) {
			$gap = ' ';
		} else {
			$gap = '';
			if ($page < 1) {
		...
		if (!TSTA::empty_string($page)) {
			for ($p = $page_first; $p <= $page_last; ++$p) {
				...
				for ($n = 1; $n <= $maxpage; ++$n) {
					...
					$na = TSTA::formatTOCPageNumber(($this->starting_page_number + $np - 1));
					$nu = TFON::UTF8ToUTF16BE($na, false, $this->isunicode, $this->CurrentFont);
					// replace aliases with numbers
					foreach ($pnalias['u'] as $u) {
						$sfill = str_repeat($filler, max(0, (strlen($u) - strlen($nu.' '))));
						if ($this->rtl) {
							$nr = $nu.TFON::UTF8ToUTF16BE(' '.$sfill, false, $this->isunicode, $this->CurrentFont);
						} else {
							$nr = TFON::UTF8ToUTF16BE($sfill.' ', false, $this->isunicode, $this->CurrentFont).$nu;
						}
	...
	public function startTransaction() {
		...
		$this->objcopy = TSTA::objclone($this);
	}
	...
	*/
	...
	protected function hyphenateWord($word, $patterns, $dictionary=array(), $leftmin=1, $rightmin=2, $charmin=1, $charmax=8) {
		...
		$word_string = TFON::UTF8ArrSubString($word, '', '', $this->isunicode);
		...
		if (isset($dictionary[$word_string])) {
			return TFON::UTF8StringToArray($dictionary[$word_string], $this->isunicode, $this->CurrentFont);
		}
		...
		for ($pos = 0; $pos < $maxpos; ++$pos) {
			...
			for ($i = 1; $i <= $imax; ++$i) {
				$subword = strtolower(TFON::UTF8ArrSubString($tmpword, $pos, ($pos + $i), $this->isunicode));
				if (isset($patterns[$subword])) {
					$pattern = TFON::UTF8StringToArray($patterns[$subword], $this->isunicode, $this->CurrentFont);
	...
	public function hyphenateText($text, $patterns, $dictionary=array(), $leftmin=1, $rightmin=2, $charmin=1, $charmax=8) {
		...
		if (!is_array($patterns)) {
			$patterns = TSTA::getHyphenPatternsFromTEX($patterns);
		}
		// get array of characters
		$unichars = TFON::UTF8StringToArray($text, $this->isunicode, $this->CurrentFont);
		// for each char
		foreach ($unichars as $char) {
			if ((!$intag) AND (!$skip) AND TFOND::$uni_type[$char] == 'L') {
				// letter character
				$word[] = $char;
			} else {
				// other type of character
				if (!TSTA::empty_string($word)) {
					// hypenate the word
		...
		return TFON::UTF8ArrSubString($txtarr, '', '', $this->isunicode);
	}
	...
	public function printTemplate($id, $x='', $y='', $w=0, $h=0, $align='', $palign='', $fitonpage=false) {
		...
		if (!empty($this->xobjects[$id]['annotations'])) {
			foreach ($this->xobjects[$id]['annotations'] as $annot) {
				...
				$coordlt = TSTA::getTransformationMatrixProduct($tm, array(1, 0, 0, 1, ($annot['x'] * $this->k), (-$annot['y'] * $this->k)));
				$ax = ($coordlt[4] / $this->k);
				$ay = ($this->h - $h - ($coordlt[5] / $this->k));
				$coordrb = TSTA::getTransformationMatrixProduct($tm, array(1, 0, 0, 1, (($annot['x'] + $annot['w']) * $this->k), ((-$annot['y'] - $annot['h']) * $this->k)));
	...
	public function ImageSVG($file, $x='', $y='', $w=0, $h=0, $link='', $align='', $palign='', $border=0, $fitonpage=false) {
		...
		if ($file[0] === '@') { // image from string
			...
			$svgdata = TSTA::fileGetContents($file);
		}
		if ($svgdata === FALSE) {
	...
	protected function setSVGStyles($svgstyle, $prevsvgstyle, $x=0, $y=0, $w=1, $h=1, $clip_function='', $clip_params=array()) {
		...
		// color
		$fill_color = TCOL::convertHTMLColorToDec($svgstyle['color'], $this->spot_colors);
		$this->SetFillColorArray($fill_color);
		// text color
		$text_color = TCOL::convertHTMLColorToDec($svgstyle['text-color'], $this->spot_colors);
		...
		if (preg_match('/url\([\s]*\#([^\)]*)\)/si', $svgstyle['fill'], $regs)) {
			...
			$fill_color = TCOL::convertHTMLColorToDec($svgstyle['fill'], $this->spot_colors);
		...
		if ($svgstyle['stroke'] != 'none') {
			...
			$stroke_style = array(
				'color' => TCOL::convertHTMLColorToDec($svgstyle['stroke'], $this->spot_colors),
					...
					foreach ($params as $ck => $cp) {
						...
						if ((($ck + 1) % 7) == 0) {
							...
							if ((abs($x0 - $x) < $minlen) AND (abs($y0 - $y) < $minlen)) {
								...
								// get angles
								$angs = TSTA::getVectorsAngle(1, 0, (($xa - $cax) / $rx), (($cay - $ya) / $ry));
								$dang = TSTA::getVectorsAngle((($xa - $cax) / $rx), (($ya - $cay) / $ry), ((-$xa - $cax) / $rx), ((-$ya - $cay) / $ry));
								if (($fs == 0) AND ($dang > 0)) {
									$dang -= (2 * M_PI);
	...
	protected function startSVGElementHandler($parser, $name, $attribs, $ctm=array()) {
		...
		if (isset($attribs['style']) AND !TSTA::empty_string($attribs['style']) AND ($attribs['style'][0] != ';')) {
			// fix style for regular expression
			$attribs['style'] = ';'.$attribs['style'];
		}
		foreach ($prev_svgstyle as $key => $val) {
			if (in_array($key, TIMA::$svginheritprop)) {
				// inherit previous value
				$svgstyle[$key] = $val;
			}
			if (isset($attribs[$key]) AND !TSTA::empty_string($attribs[$key])) {
			...
			} elseif (isset($attribs['style']) AND !TSTA::empty_string($attribs['style'])) {
				// CSS style syntax
				$attrval = array();
		...
		if (isset($attribs['transform']) AND !empty($attribs['transform'])) {
			$tm = TSTA::getTransformationMatrixProduct($tm, TSTA::getSVGTransformMatrix($attribs['transform']));
		}
		...
		switch($name) {
			...
			case 'svg': {
				...
				$tm = TSTA::getTransformationMatrixProduct($tm, array( 1, 0, 0, 1, $svgX, $svgY));
				...
				if (isset($attribs['viewBox'])) {
					...
					if (sizeof($tmp) == 4) {
						...
						$tm = TSTA::getTransformationMatrixProduct($tm, $newtm);
						$this->SVGTransform($tm);
					}
				}
			...
			case 'g': {
				...
				$tm = TSTA::getTransformationMatrixProduct($tm, array($w, 0, 0, $h, $x, $y));
				$this->SVGTransform($tm);
				$this->setSVGStyles($svgstyle, $prev_svgstyle);
				break;
			}
			case 'linearGradient': {
				...
				if (isset($attribs['gradientTransform'])) {
					$this->svggradients[$this->svggradientid]['gradientTransform'] = TSTA::getSVGTransformMatrix($attribs['gradientTransform']);
				}
			...
			case 'stop': {
				...
				$stop_color = isset($svgstyle['stop-color'])?TCOL::convertHTMLColorToDec($svgstyle['stop-color'], $this->spot_colors):'black';
			...
			case 'image': {
				...
				if (!$clipping) {
					...
					if (preg_match('/^data:image\/[^;]+;base64,/', $img, $m) > 0) {
						...
						// fix image path
						if (!TSTA::empty_string($this->svgdir) AND (($img[0] == '.') OR (basename($img) == $img))) {
					...
					// get image type
					$imgtype = TIMA::getImageFileType($img);
					if (($imgtype == 'eps') OR ($imgtype == 'ai')) {
