...
class WebPayNormal {
    ...
    function __construct($config) {
        ...
        $publicCert = $this->config["PUBLIC_CERT"];
    ...
    public function initTransaction($amount, $sessionId, $ordenCompra, $urlFinal){
        try{
            ...
            $wsInitTransactionInput->returnURL = $this->config["URL_RETURN"];
            $wsInitTransactionInput->finalURL = $urlFinal;
            $wsTransactionDetail = new wsTransactionDetail();
            $wsTransactionDetail->commerceCode = $this->config["COMMERCE_CODE"];
            ...
            $initTransactionResponse = $this->_initTransaction(array("wsInitTransactionInput" => $wsInitTransactionInput));
            $xmlResponse = $this->soapClient->__getLastResponse();
            $soapValidation = new SoapValidation($xmlResponse, $this->config["WEBPAY_CERT"]);
            $validationResult = $soapValidation->getValidationResult();
            if ($validationResult === TRUE) {
                $wsInitTransactionOutput = $initTransactionResponse->return;
                $this->log->writeLog('initTransaction',$wsTransactionDetail->buyOrder ,$wsTransactionDetail, $wsInitTransactionOutput, true);
                return array (
        ...
        $this->log->writeLog('initTransaction', $wsTransactionDetail->buyOrder ,$wsTransactionDetail, $error, false);
        return $error;
    }
        ...
        if ($validationResult === TRUE) {
            ...
            if ($this->acknowledgeTransaction($token)) {
                ...
                if ( ($result->VCI == "TSY" || $result->VCI == "A" || $result->VCI == "") && $resultCode == 0) {
                    $this->log->writeLog('getTransactionResult', $result->buyOrder, $token, $result->detailOutput, true);
                    return $result;
                } else {
                ...
                $this->log->writeLog('getTransactionResult', $result->buyOrder, $token,"Error eviando ACK a Webpay", false );
                return array("error" => "Error eviando ACK a Webpay");
            }
        }
        $this->log->writeLog('getTransactionResult',$result->buyOrder, $token, "Error validando transacción en Webpay", false );
    ...
    public function acknowledgeTransaction($token){
        ...
        $soapValidation = new SoapValidation($xmlResponse, $this->config["WEBPAY_CERT"]);
        $validationResult = $soapValidation->getValidationResult();
        $this->log->writeLog('acknowledgeTransaction', null, $token, $validationResult, true );
